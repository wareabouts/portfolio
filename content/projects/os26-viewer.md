...
I built it solo over about a month, pair programming with Claude Code. The team started using it for real when it was maybe 60% done, which kept things exciting.
...
Every sign at the event went through it. The real b2Sign order came straight off the order sheet, and volunteers checked signs off while walking the venue. One Figma file ended up driving the venue model, the artwork AND the purchase order.

Next time? One Firestore doc per sign from day one instead of migrating mid-project, and build the volunteer checklist BEFORE install week instead of during. The [code is on GitHub](https://github.com/wareabouts/os26-viewer).
